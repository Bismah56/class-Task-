...
//                           Basic Arrays  --- Product Inventory System
// Defining type alias 
type ProductDetails = {
        name : string,
        price : number,
        ...
        inventory : {
            stock : number,
            availableColors : [string,string?,string?,string?,string?,string?]
        }
};
...
function changeColor( productName:string, color:string ) {
...
//                                        Multi-Dimensional Arrays and Tuples-Students Grading System
// Defining type alias 
type Student = {
    name : string;
    marks : any[],
...
function calculateAverageGrade(studentName:string) {
     if(students.some((student)=>{return studentName===student.name})){
         let selectedStudent:any = students.find((element)=>{return studentName===element.name});
         
         //Calculating Average of students
         let SumOfMarks = selectedStudent?.marks.reduce((prev:any,curr:any)=>{
            let sum = (prev+curr);
            return sum;
         });
...
//                            Array wwith Type and Indexing -- Employee Salaries Management
type Employee = {
    name : string,
    hoursWorked :number,
    hourlyRate : number,
...
function calculateSalariesAndBonus (EmployeeName:string) {
    if(employees.some((element)=>{return EmployeeName===element.name})){

        let EMPLOYEE:any = employees.find((Element)=>{return EmployeeName===Element.name});
